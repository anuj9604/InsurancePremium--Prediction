...
class ModelEvaluation:
    ...
    def get_best_model(self):
        """Gets the model object of the model that is in production"""
        try:
            model = None
    ...
    def update_evaluation_report(
        ...
        """
        Given that we need to update the model in production with the one we trained
        """
        try:
            ...
            if previous_best_model is not None:
                model_history = {
                    self.model_evaluation_config.timestamp: previous_best_model
                }
    ...
    def initiate_model_evaluation(self) -> ModelEvaluationArtifact:
        try:
            ...
            target_column_name = schema_content[TARGET_COLUMNS]

            train_target_dataframe = train_dataframe[target_column_name]
            test_target_dataframe = test_dataframe[target_column_name]
            train_dataframe = train_dataframe.drop(
                target_column_name, axis=1, inplace=True
            )
            ...
            if model is None:
                logging.info(
                    "Not found any existing model. Hence accepting the trained model"
                )
            ...
            metric_info_artifact = evaluate_regression_model(
                ...
                y_train=train_target_dataframe,
                X_test=test_dataframe,
                y_test=test_target_dataframe,
