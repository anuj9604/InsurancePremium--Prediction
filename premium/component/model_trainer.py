...
from premium.util.util import save_object, load_object, load_numpy_array_data
from premium.entity.model_factory import (
    MetricInfoArtifact,
    ModelFactory,
...
class PremiumEstimatorModel:
    ...
    def predict(self, X):
        """
        function accepts raw inputs and then transforms raw input using preprocessing_object which guarantees that the inputs are in the same format as the training data. At last it performs prediction on transformed features
        """
...
class ModelTrainer:
    def __init__(
        ...
        try:
            logging.info(f"{'='*20}Model Triner log started{'='*20}")
    ...
    def initiate_model_trainer(self) -> ModelTrainerArtifact:
        try:
            ...
            logging.info(f"Bestmodel found on training dataset: {best_model}")
            ...
            logging.info(
                "Evaluation of all trained models on training and testing data"
            )
            ...
            logging.info(
                f"Best found model on both training and testing dataset, model {metric_info.model_name}"
            )
            ...
            premium_model = PremiumEstimatorModel(
