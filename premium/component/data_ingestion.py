...
class DataIngestion:
    ...
    def download_insurance_data(self):
        try:
            ...
            df = pd.read_html(csv_url)
            df[0].drop("Unnamed: 0", axis=1, inplace=True)
            df[0].to_csv(csv_file_path, index=False)
            logging.info(
                f"csv_file {insurance_file_name} has been downloaded successfullt to: {csv_file_path}"
            )
    ...
    def train_test_split(self):
        try:
            ...
            premium_df["category"] = pd.cut(
                premium_df["expenses"],
                bins=[0, 11563.5, 22005, 32446.5, 42888, 53329.5, np.inf],
                labels=[0, 1, 2, 3, 4, 5],
            )
            ...
            for train_index, test_index in split.split(
                premium_df, premium_df["category"]
            ):
